                            ...
                            if p_resp.status_code == 200:
                                ...
                                if sms_list:
                                    ...
                                    for sms in sms_list:
                                        ...
                                        if sms_text not in received_sms_texts:
                                            received_sms_texts.add(sms_text)
                                            new_sms_found = True
                                            otp_val = sms.get('code', 'N/A')
                                            full_sms = sms_text
                                            
                                            # Update state: this order has received an OTP
                                            users_db[user_id].setdefault('orders_with_otp', set()).add(str(order_id))
                                            save_db(db_data)

                                            # Log success to external group (only for new ones)
                                            try:
                                                user_info_log = str(user_id)
                                                try:
                                                ...
                                                if group_resp.status_code != 200:
                                                    print(f"Group Log Error: {group_resp.text}")
                                            except Exception as ge: 
                                                print(f"Group send exception: {ge}")

                                            # Send separate notification message for each new SMS
                                            bot.send_message(
                                                chat_id,
                                                f"🎯 *SMS Received!*\n\n"
                                                f"📱 *Phone:* `{phone}`\n"
                                    ...
                                    if new_sms_found:
                                        # Update the main message with latest status (ONLY IF ACTIVE THREAD)
                                        if users_db[user_id].get('active_threads', {}).get(msg_id) == thread_token:
                                            ...
                                            done_text = (f"✅ *Number Found!*\n\n"
                                                         ...
                                                         f"📨 *Latest SMS:* `{full_sms}`\n\n"
                                                         f"🟢 *Status:* {status_text}\n✅ SMS Received ({len(received_sms_texts)})!")
                                ...
                                # Update elapsed time display (ONLY IF ACTIVE THREAD)
                                if users_db[user_id].get('active_threads', {}).get(msg_id) == thread_token:
                                    try:
                                        # Update wait_text with latest OTP if it arrived
                                        display_otp = otp_val
                                        temp_wait_text = (f"✅ *Number Found!*\n\n"
                                                         f"📱 *Phone:* `{phone}`\n"
                                                         f"📦 *Service:* {service.capitalize()}\n"
                                                         f"💰 *Price:* {data.get('price')} ₽\n"
                                                         f"🔑 *OTP:* `{display_otp}`\n\n"
...
def cancel_order_again(call):
    ...
    # NEW LOGIC: Cancel only if NO OTP received
    ...
    if not has_otp:
        ...
        bot.answer_callback_query(call.id, "❌ No OTP received. Number cancelled. Searching again...")
    else:
        bot.answer_callback_query(call.id, "✅ OTP received. Keeping number active. Searching for next...")
